...
func NewRedisRepo[t any](addr, password, setName string) repository[t] {
...
func (r repository[t]) SaveForReccal(ctx context.Context, item Item[t]) error {
	return r.client.HSet(ctx, r.setName, item.Key, item).Err()
}

func (r repository[t]) UpdateItems(ctx context.Context, items []Item[t]) (err error) {
...
func (r repository[t]) GetItemsForResend(ctx context.Context) ([]Item[t], error) {
	result := r.client.HGetAll(ctx, r.setName)
	if result.Err() != nil {
		return nil, result.Err()
	}

	res := make([]Item[t], 0, 10)
...
func (r repository[t]) DeleteItems(ctx context.Context, items []Item[t]) error {
